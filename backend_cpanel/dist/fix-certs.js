...
async function main() {
    ...
    for (const reg of registrations) {
        ...
        const pdfPath = await (0, certificateService_1.generateCertificate)(reg.user.name, 'B.Tech', reg.user.college || 'Engineering College', reg.internship.domain, reg.startDate.toISOString(), reg.endDate.toISOString(), certId);
        if (reg.certificate) {
