 ...
 */
async function updateRegistrationRow(email, updates) {
    try {
        const sheet = await getSheet();
        if (!sheet)
            return;
        ...
        for (let i = rows.length - 1; i >= 0; i--) {
            ...
            if (row['Email'] === email) {
                ...
                if (updates.taskLink)
                    row['Task Link'] = updates.taskLink;
                if (updates.certificateLink)
                    row['Certificate Link'] = updates.certificateLink;
                await row.save();
